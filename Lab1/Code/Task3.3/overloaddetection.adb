...
task type Watchdog(Prio: Integer) is
    pragma Priority(Prio);  -- Highest priority
	entry Signal;
end Watchdog;

task body Watchdog is
begin
delay until Start + Milliseconds(Warm_Up_Time); -- Wait for warmup time
    loop
         select
			accept Signal; 
         or
			...
			-- => Timeout
			Put_Line("Watchdog Timeout!"); -- => Report Timeout
			-- do something
         end select;
      end loop;
end Watchdog;

-- Define the tasks based on Γ1
Task_Watchdog : Watchdog(40);
Task_1 : T(1, 30, Warm_Up_Time, 300, 100, 300); -- ID: 1, Priority: 30, Phase: 100, Period: 300, Computation Time: 100, Relative Deadline: 300
Task_2 : T(2, 20, Warm_Up_Time, 400, 100, 400);
Task_3 : T(3, 10, Warm_Up_Time, 600, 100, 600);
Task_4 : T(4, 5, Warm_Up_Time, 1200, 300, 1200); -- C(T4) is changed to 300 to fill the hyperperiod (no slack time)

task Helper is
    pragma Priority(0);  -- Lowest priority
end Helper;
...
delay until Start + Milliseconds(Warm_Up_Time); -- Wait for warmup time
    loop
		delay To_Duration(Milliseconds(5)); -- arbitrary small delay
		Task_Watchdog.Signal; -- => Signal watchdog
    end loop;
end Helper;
